...
class IMP(nn.Module):
    ...
    def __init__(self, cfg, in_channels):
        ...
        if self.update_step > 0:
            ...
            self.subj_node_gate = nn.Sequential(
                nn.Linear(self.dim * 2, 1), nn.Sigmoid())
    ...
    def forward(self, features, proposals, proposal_pairs):
        ...
        rel_inds, subj_pred_map, obj_pred_map = self._get_map_idxs(
            proposals, proposal_pairs)
        x_obj = torch.cat([proposal.get_field("box_features")
        ...
        for t in range(self.update_step):
            ...
            message_pred_to_subj = self.subj_node_gate(
                torch.cat([sub_vert, hx_edge[t]], 1)) * hx_edge[t]  # nrel x d
            message_pred_to_obj = self.obj_node_gate(
                torch.cat([obj_vert, hx_edge[t]], 1)) * hx_edge[t]    # nrel x d
        ...
        obj_class_logits = self.obj_predictor(
            hx_obj[-1].unsqueeze(2).unsqueeze(3))
        pred_class_logits = self.pred_predictor(
            hx_edge[-1].unsqueeze(2).unsqueeze(3))
