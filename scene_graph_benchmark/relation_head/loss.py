...
class FastRCNNLossComputation(object):
    ...
    def match_targets_to_proposals(self, proposal, target):
        ...
        for i in range(match_quality_matrix.shape[0]):
            for j in range(match_quality_matrix.shape[0]):
                ...
                non_duplicate_idx = (torch.eye(
                    match_ij.shape[0]).view(-1) == 0).nonzero(as_tuple=False).view(-1).to(match_ij.device)
                # [::match_quality_matrix.shape[1]] = 0
        ...
        target_pair.add_field(
            "labels", target.get_field("pred_labels").view(-1))
        ...
        idx_subj = torch.arange(
            box_subj.shape[0]).view(-1, 1, 1).repeat(1, box_obj.shape[0], 1).to(proposal.bbox.device)
        idx_obj = torch.arange(box_obj.shape[0]).view(
            1, -1, 1).repeat(box_subj.shape[0], 1, 1).to(proposal.bbox.device)
        proposal_idx_pairs = torch.cat(
            (idx_subj.view(-1, 1), idx_obj.view(-1, 1)), 1)

        non_duplicate_idx = (
            proposal_idx_pairs[:, 0] != proposal_idx_pairs[:, 1]).nonzero(as_tuple=False)
        ...
        proposal_pairs = BoxPairList(
            proposal_box_pairs, proposal.size, proposal.mode)
    ...
    def RelPN_GenerateProposalLabels(self, cfg, proposals, targets, proposal_box_pairs):
        ...
        for i in range(len(proposals)):
            output_blob_names = ['sbj_rois', 'obj_rois', 'rel_rois',
                                 'fg_prd_labels_int32', 'all_prd_labels_int32', 'fg_size']
    ...
    def contrastive_loss_sample(self, cfg, proposals, targets):
        ...
        box_subj = box_subj.unsqueeze(1).repeat(1, box_subj.shape[0], 1)
        box_obj = box_obj.unsqueeze(0).repeat(box_obj.shape[0], 1, 1)
        proposal_box_pairs_per_image = torch.cat(
            (box_subj.view(-1, 4), box_obj.view(-1, 4)), 1)

        idx_subj = torch.arange(box_subj.shape[0]).view(-1, 1, 1).repeat(
            1, box_obj.shape[0], 1).to(proposals[0].bbox.device)
        idx_obj = torch.arange(box_obj.shape[0]).view(
            1, -1, 1).repeat(box_subj.shape[0], 1, 1).to(proposals[0].bbox.device)
        proposal_idx_pairs_per_image = torch.cat(
            (idx_subj.view(-1, 1), idx_obj.view(-1, 1)), 1)

        keep_idx = (proposal_idx_pairs_per_image[:, 0] != proposal_idx_pairs_per_image[:, 1]).nonzero(
        ...
        blobs_out = self.RelPN_GenerateProposalLabels(
            cfg, proposals, targets, proposal_box_pairs)

        # get proposal_pairs
        proposal_pairs = []
        for i, (proposals_per_image, blob) in enumerate(zip(proposals, blobs_out)):
            # boxes and labels
            all_box_pairs = np.concatenate(
                (blob['sbj_rois'], blob['obj_rois']), axis=1)
            all_label_pairs = np.concatenate((blob['all_sbj_labels_int32'].reshape(
                (-1, 1)), blob['all_obj_labels_int32'].reshape(-1, 1)), axis=1)
            all_prd_label = blob['all_prd_labels_int32']

            sub_pos_box_pairs = np.concatenate(
                (blob['sbj_rois_sbj_pos'], blob['obj_rois_sbj_pos']), axis=1)
            sub_pos_label_pairs = np.concatenate((blob['sbj_labels_sbj_pos_int32'].reshape(
                (-1, 1)), blob['obj_labels_sbj_pos_int32'].reshape(-1, 1)), axis=1)
            sub_pos_prd_label = blob['prd_labels_sbj_pos_int32']

            obj_pos_box_pairs = np.concatenate(
                (blob['sbj_rois_obj_pos'], blob['obj_rois_obj_pos']), axis=1)
            obj_pos_label_pairs = np.concatenate((blob['sbj_labels_obj_pos_int32'].reshape(
                (-1, 1)), blob['obj_labels_obj_pos_int32'].reshape(-1, 1)), axis=1)
            obj_pos_prd_label = blob['prd_labels_obj_pos_int32']

            proposal_box_pairs = np.concatenate(
                (all_box_pairs, sub_pos_box_pairs, obj_pos_box_pairs), axis=0)
            proposal_label_pairs = np.concatenate(
                (all_label_pairs, sub_pos_label_pairs, obj_pos_label_pairs), axis=0)
            proposal_prd_label_pairs = np.concatenate(
                (all_prd_label, sub_pos_prd_label, obj_pos_prd_label), axis=0)
            proposal_binary_all_pairs = np.zeros(
                proposal_box_pairs.shape[0], dtype=np.int32)
            proposal_binary_all_pairs[:all_box_pairs.shape[0]] = 1
            proposal_binary_label_sub_pairs = np.zeros(
                proposal_box_pairs.shape[0], dtype=np.int32)
            proposal_binary_label_sub_pairs[all_box_pairs.shape[0]:all_box_pairs.shape[0] + sub_pos_box_pairs.shape[0]] = 1
            proposal_binary_label_obj_pairs = np.zeros(
                proposal_box_pairs.shape[0], dtype=np.int32)
            proposal_binary_label_obj_pairs[all_box_pairs.shape[0]
                                            + sub_pos_box_pairs.shape[0]:] = 1

            # transform to cuda
            device = proposals_per_image.bbox.device

            proposal_box_pairs = torch.FloatTensor(
                proposal_box_pairs).to(device)
            proposal_label_pairs = torch.LongTensor(
                proposal_label_pairs).to(device)
            proposal_prd_label_pairs = torch.LongTensor(
                proposal_prd_label_pairs).to(device)
            proposal_binary_all_pairs = torch.LongTensor(
                proposal_binary_all_pairs).to(device)
            proposal_binary_label_sub_pairs = torch.LongTensor(
                proposal_binary_label_sub_pairs).to(device)
            proposal_binary_label_obj_pairs = torch.LongTensor(
                proposal_binary_label_obj_pairs).to(device)
            ...
            proposal_pairs_per_image.add_field(
                "prd_label_pairs", proposal_prd_label_pairs)
            proposal_pairs_per_image.add_field(
                "binary_all_pairs", proposal_binary_all_pairs)
            proposal_pairs_per_image.add_field(
                "binary_label_sub_pairs", proposal_binary_label_sub_pairs)
            proposal_pairs_per_image.add_field(
                "binary_label_obj_pairs", proposal_binary_label_obj_pairs)

            # saved for contrastive losses (numpy)
            proposal_pairs_per_image.add_field(
                "binary_labels_sbj_pos_int32", blob['binary_labels_sbj_pos_int32'])
            proposal_pairs_per_image.add_field(
                "inds_unique_sbj_pos", blob['inds_unique_sbj_pos'])
            proposal_pairs_per_image.add_field(
                "inds_reverse_sbj_pos", blob['inds_reverse_sbj_pos'])
            proposal_pairs_per_image.add_field(
                "binary_labels_obj_pos_int32", blob['binary_labels_obj_pos_int32'])
            proposal_pairs_per_image.add_field(
                "inds_unique_obj_pos", blob['inds_unique_obj_pos'])
            proposal_pairs_per_image.add_field(
                "inds_reverse_obj_pos", blob['inds_reverse_obj_pos'])
            proposal_pairs_per_image.add_field(
                "sbj_labels_sbj_pos_int32", blob['sbj_labels_sbj_pos_int32'])
            proposal_pairs_per_image.add_field(
                "obj_labels_sbj_pos_int32", blob['obj_labels_sbj_pos_int32'])
            proposal_pairs_per_image.add_field(
                "sbj_labels_obj_pos_int32", blob['sbj_labels_obj_pos_int32'])
            proposal_pairs_per_image.add_field(
                "obj_labels_obj_pos_int32", blob['obj_labels_obj_pos_int32'])
            proposal_pairs_per_image.add_field(
                "prd_labels_sbj_pos_int32", blob['prd_labels_sbj_pos_int32'])
            proposal_pairs_per_image.add_field(
                "prd_labels_obj_pos_int32", blob['prd_labels_obj_pos_int32'])
    ...
    def contrastive_proposal_pair_transform(self, proposals, proposal_pairs):
        for proposal_per_image, proposal_pairs_per_image in zip(proposals, proposal_pairs):
            ...
            assert torch.min(
                vals) > 0.99, "Some sub/obj is not from proposals/targets!"

            rel_ind_i = torch.cat(
                (inds[0::2].view(-1, 1), inds[1::2].view(-1, 1)), 1).to(device)
    ...
    def __call__(self, class_logits):
        ...
        classification_loss = F.cross_entropy(
            class_logits, labels, weight=ce_weights)
    ...
    def obj_classification_loss(self, proposals, class_logits):
        class_logits = cat(class_logits, dim=0)
        device = class_logits.device
        # labels = cat([proposal.get_field("gt_labels")
        #              for proposal in proposals], dim=0)
        labels = cat([proposal.get_field("labels")
                     for proposal in proposals], dim=0)
    ...
    def cross_entropy_losses(self, class_logits):
        ...
        prd_label_pairs = cat([proposal.get_field("prd_label_pairs")
                              for proposal in proposals], dim=0)
        binary_all_pairs = cat([proposal.get_field("binary_all_pairs")
                               for proposal in proposals], dim=0)
    ...
    def reldn_contrastive_losses(self, cfg, class_logits, margin=0.2):
        ...
        prd_label_pairs = cat([proposal.get_field("prd_label_pairs")
                              for proposal in proposals], dim=0)
        binary_label_sub_pairs = cat(
            [proposal.get_field("binary_label_sub_pairs") for proposal in proposals], dim=0)
        binary_label_obj_pairs = cat(
            [proposal.get_field("binary_label_obj_pairs") for proposal in proposals], dim=0)
        prd_scores_sbj_pos = class_logits[binary_label_sub_pairs == 1]
        prd_scores_obj_pos = class_logits[binary_label_obj_pairs == 1]

        binary_labels_sbj_pos_int32 = np.concatenate([proposal.get_field(
            "binary_labels_sbj_pos_int32") for proposal in proposals], axis=0)
        inds_unique_sbj_pos = np.concatenate(
            [proposal.get_field("inds_unique_sbj_pos") for proposal in proposals], axis=0)
        inds_reverse_sbj_pos = np.concatenate(
            [proposal.get_field("inds_reverse_sbj_pos") for proposal in proposals], axis=0)
        binary_labels_obj_pos_int32 = np.concatenate([proposal.get_field(
            "binary_labels_obj_pos_int32") for proposal in proposals], axis=0)
        inds_unique_obj_pos = np.concatenate(
            [proposal.get_field("inds_unique_obj_pos") for proposal in proposals], axis=0)
        inds_reverse_obj_pos = np.concatenate(
            [proposal.get_field("inds_reverse_obj_pos") for proposal in proposals], axis=0)
    ...
    def reldn_so_contrastive_losses(self, cfg, class_logits, margin=0.2):
        ...
        inds_reverse_obj_pos = np.concatenate(
            [proposal.get_field("inds_reverse_obj_pos") for proposal in proposals], axis=0)
        sbj_labels_sbj_pos_int32 = np.concatenate([proposal.get_field(
            "sbj_labels_sbj_pos_int32") for proposal in proposals], axis=0)
        obj_labels_sbj_pos_int32 = np.concatenate([proposal.get_field(
            "obj_labels_sbj_pos_int32") for proposal in proposals], axis=0)
        sbj_labels_obj_pos_int32 = np.concatenate([proposal.get_field(
            "sbj_labels_obj_pos_int32") for proposal in proposals], axis=0)
        obj_labels_obj_pos_int32 = np.concatenate([proposal.get_field(
            "obj_labels_obj_pos_int32") for proposal in proposals], axis=0)
    ...
    def reldn_p_contrastive_losses(self, cfg, class_logits, margin=0.2):
        ...
        inds_reverse_obj_pos = np.concatenate(
            [proposal.get_field("inds_reverse_obj_pos") for proposal in proposals], axis=0)
        prd_labels_sbj_pos_int32 = np.concatenate([proposal.get_field(
            "prd_labels_sbj_pos_int32") for proposal in proposals], axis=0)
        prd_labels_obj_pos_int32 = np.concatenate([proposal.get_field(
            "prd_labels_obj_pos_int32") for proposal in proposals], axis=0)
    ...
    def split_pos_neg_spo_agnostic(self, prd_probs, binary_labels_pos, inds_unique_pos, inds_reverse_pos):
        ...
        pair_pos_batch = torch.ones(1).cuda(device_id)
        # a dummy sample in the batch in case there is no real sample
        pair_neg_batch = torch.zeros(1).cuda(device_id)
        for i in range(inds_unique_pos.shape[0]):
            ...
            if pair_pos_inds.size == 0 or pair_neg_inds.size == 0:
                continue
    ...
    def split_pos_neg_so_aware(self, cfg, prd_probs, binary_labels_pos, inds_unique_pos, inds_reverse_pos, sbj_labels_pos, obj_labels_pos, s_or_o):
        ...
        for i in range(inds_unique_pos.shape[0]):
            ...
            max_prd_pos_probs_i_pair_neg = torch.max(
                prd_pos_probs_i_pair_neg)  # this is fixed for a given i
            if s_or_o == 's':
                ...
                for j in range(inds_unique_obj_labels.shape[0]):
                    ...
                    # get max neg
                    neg_j_inds = np.where(
                        obj_labels_i_pair_neg == unique_obj_labels[j])[0]
                ...
                for j in range(inds_unique_sbj_labels.shape[0]):
                    ...
                    # get max neg
                    neg_j_inds = np.where(
                        sbj_labels_i_pair_neg == unique_sbj_labels[j])[0]
                    if neg_j_inds.size == 0:
                        if cfg.MODEL.ROI_RELATION_HEAD.CONTRASTIVE_LOSS.USE_SPO_AGNOSTIC_COMPENSATION:
                            pair_pos_batch = torch.cat(
                                (pair_pos_batch, min_prd_pos_probs_i_pos_j.unsqueeze(0)))
    ...
    def split_pos_neg_p_aware(self, cfg, prd_probs, prd_bias_probs, binary_labels_pos, inds_unique_pos, inds_reverse_pos, prd_labels_pos):
        ...
        if len(prd_probs) == 0:
            ...
            prd_labels_det = prd_probs[:, 1:].argmax(
        ...
        for i in range(inds_unique_pos.shape[0]):
            ...
            for j in range(inds_unique_prd_labels.shape[0]):
                ...
                # get max neg
                neg_j_inds = np.where(
                    prd_labels_i_pair_neg == unique_prd_labels[j])[0]
                if neg_j_inds.size == 0:
                    if cfg.MODEL.ROI_RELATION_HEAD.CONTRASTIVE_LOSS.USE_SPO_AGNOSTIC_COMPENSATION:
                        pair_pos_batch = torch.cat(
                            (pair_pos_batch, min_prd_pos_probs_i_pos_j.unsqueeze(0)))
