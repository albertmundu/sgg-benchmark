...
class RelDN(nn.Module):
    ...
    def _get_map_idxs(self, proposals, proposal_pairs):
        ...
        subj_pred_map = rel_inds.new(sum(
            [len(proposal) for proposal in proposals]), rel_inds.shape[0]).fill_(0).float().detach()
        obj_pred_map = rel_inds.new(sum(
            [len(proposal) for proposal in proposals]), rel_inds.shape[0]).fill_(0).float().detach()
    ...
    def forward(self, features, proposals, proposal_pairs):
        ...
        x_pred, _ = self.pred_feature_extractor(
            features, proposals, proposal_pairs, use_relu=False)
        ...
        for img_id, (proposal_per_image, proposal_pairs_per_image) in \
            ...
            if self.cfg.MODEL.ROI_RELATION_HEAD.SEPERATE_SO_FEATURE_EXTRACTOR:
                ...
                sub_vert_per_image = proposal_per_image.get_field("subj_box_features")[
                    rel_ind_i[:, 0]]
                obj_vert_per_image = proposal_per_image.get_field("obj_box_features")[
                    rel_ind_i[:, 1]]
            else:
            ...
            class_logits_per_image = self.freq_bias.index_with_labels(
                torch.stack((subj_vert_labels, obj_vert_labels,), 1))
        ...
        rel_subj_class_logits = self.rel_subj_predictor(
            sub_vert.unsqueeze(2).unsqueeze(3))
        rel_obj_class_logits = self.rel_obj_predictor(
            obj_vert.unsqueeze(2).unsqueeze(3))

        x_rel = torch.cat([sub_vert, x_pred, obj_vert], 1)
        x_rel = self.rel_embedding(x_rel)
        rel_pred_class_logits = self.rel_pred_predictor(
            x_rel.unsqueeze(2).unsqueeze(3))
        ...
        '''sum up all scores'''
        rel_class_logits = rel_vis_class_logits + \
            rel_sem_class_logits + rel_spt_class_logits

        if obj_class_logits is None:
            obj_class_labels = torch.cat(
                [proposal.get_field("labels").detach() for proposal in proposals], 0)
        else:
