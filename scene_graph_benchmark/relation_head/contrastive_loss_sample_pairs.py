...
#         logger.info('{} : {}'.format(fg_pair_inds.size, bg_pair_inds.size))
        ...
        all_prd_labels[:fg_pair_inds.size] = fg_prd_labels + \
            1  # class should start from 1
    else:
        keep_pair_inds = fg_pair_inds
        all_prd_labels = fg_prd_labels
    blob_dict['all_prd_labels_int32'] = all_prd_labels.astype(
        np.int32, copy=False)
    # this is used to check if there is at least one fg to learn
    blob_dict['fg_size'] = np.array([fg_pair_inds.size], dtype=np.int32)
    ...
    if cfg.MODEL.ROI_RELATION_HEAD.CONTRASTIVE_LOSS.USE_FREQ_BIAS:
        sbj_labels = targets_per_image.get_field(
            'max_sbj_classes')[keep_pair_inds]
        obj_labels = targets_per_image.get_field(
            'max_obj_classes')[keep_pair_inds]
        blob_dict['all_sbj_labels_int32'] = sbj_labels.astype(
            np.int32, copy=False)
        blob_dict['all_obj_labels_int32'] = obj_labels.astype(
            np.int32, copy=False)
    if cfg.MODEL.ROI_RELATION_HEAD.CONTRASTIVE_LOSS.USE_NODE_CONTRASTIVE_LOSS or \
        ...
        sbj_pos_pair_pos_inds = np.where(
            (max_pair_overlaps >= cfg.MODEL.ROI_RELATION_HEAD.CONTRASTIVE_LOSS.FG_THRESH))[0]
        ...
        blob_dict['binary_labels_sbj_pos_int32'] = binary_labels_sbj_pos.astype(
            np.int32, copy=False)
        prd_pos_labels_sbj_pos = targets_per_image.get_field('max_prd_classes')[
            sbj_pos_pair_pos_inds]
        ...
        blob_dict['prd_labels_sbj_pos_int32'] = prd_labels_sbj_pos.astype(
            np.int32, copy=False)
        sbj_labels_sbj_pos = targets_per_image.get_field('max_sbj_classes')[
            sbj_pos_inds] + 1
        # 1. set all obj labels > 0
        obj_labels_sbj_pos = targets_per_image.get_field('max_obj_classes')[
            sbj_pos_inds] + 1
        # 2. find those negative obj
        max_obj_overlaps_sbj_pos = targets_per_image.get_field('max_obj_overlaps')[
            sbj_pos_inds]
        obj_neg_inds_sbj_pos = np.where(
            max_obj_overlaps_sbj_pos < cfg.MODEL.ROI_RELATION_HEAD.CONTRASTIVE_LOSS.FG_THRESH)[0]
        ...
        blob_dict['sbj_labels_sbj_pos_fg_int32'] = targets_per_image.get_field(
            'max_sbj_classes')[sbj_pos_inds].astype(np.int32, copy=False)
        blob_dict['obj_labels_sbj_pos_fg_int32'] = targets_per_image.get_field(
            'max_obj_classes')[sbj_pos_inds].astype(np.int32, copy=False)

        sampled_sbj_boxes_sbj_pos = targets_per_image.get_field('sbj_boxes')[
            sbj_pos_inds]
        sampled_obj_boxes_sbj_pos = targets_per_image.get_field('obj_boxes')[
            sbj_pos_inds]
        # Scale rois and format as (x1, y1, x2, y2)
        ...
        sampled_rel_rois_sbj_pos = rois_union(
        ...
        blob_dict['binary_labels_obj_pos_int32'] = binary_labels_obj_pos.astype(
            np.int32, copy=False)
        prd_pos_labels_obj_pos = targets_per_image.get_field('max_prd_classes')[
            obj_pos_pair_pos_inds]
        ...
        blob_dict['prd_labels_obj_pos_int32'] = prd_labels_obj_pos.astype(
            np.int32, copy=False)
        obj_labels_obj_pos = targets_per_image.get_field('max_obj_classes')[
            obj_pos_inds] + 1
        # 1. set all sbj labels > 0
        sbj_labels_obj_pos = targets_per_image.get_field('max_sbj_classes')[
            obj_pos_inds] + 1
        # 2. find those negative sbj
        max_sbj_overlaps_obj_pos = targets_per_image.get_field('max_sbj_overlaps')[
            obj_pos_inds]
        sbj_neg_inds_obj_pos = np.where(
            max_sbj_overlaps_obj_pos < cfg.MODEL.ROI_RELATION_HEAD.CONTRASTIVE_LOSS.FG_THRESH)[0]
        ...
        blob_dict['obj_labels_obj_pos_int32'] = obj_labels_obj_pos.astype(
            np.int32, copy=False)
        # this is for freq bias in RelDN
        blob_dict['sbj_labels_obj_pos_fg_int32'] = targets_per_image.get_field(
            'max_sbj_classes')[obj_pos_inds].astype(np.int32, copy=False)
        blob_dict['obj_labels_obj_pos_fg_int32'] = targets_per_image.get_field(
            'max_obj_classes')[obj_pos_inds].astype(np.int32, copy=False)

        sampled_sbj_boxes_obj_pos = targets_per_image.get_field('sbj_boxes')[
            obj_pos_inds]
        sampled_obj_boxes_obj_pos = targets_per_image.get_field('obj_boxes')[
            obj_pos_inds]
        # Scale rois and format as (x1, y1, x2, y2)
        ...
        sampled_rel_rois_obj_pos = rois_union(
...
def add_rel_proposals(proposals, targets, proposal_box_pairs):
    ...
    _merge_paired_boxes_into_roidb(
        proposals, targets, sbj_box_list, obj_box_list)
...
def _merge_paired_boxes_into_roidb(proposals, targets, sbj_box_list, obj_box_list):
    assert len(sbj_box_list) == len(obj_box_list) == len(
        proposals) == len(targets) == 1
    for i, (proposals_per_image, targets_per_image) in enumerate(zip(proposals, targets)):
        ...
        sbj_gt_overlaps = np.zeros(
            (num_pairs, targets_per_image.get_field(
                'sbj_gt_overlaps').shape[1]),
            dtype=targets_per_image.get_field(
                'sbj_gt_overlaps').cpu().numpy().dtype
        )
        obj_gt_overlaps = np.zeros(
            (num_pairs, targets_per_image.get_field(
                'obj_gt_overlaps').shape[1]),
            dtype=targets_per_image.get_field(
                'obj_gt_overlaps').cpu().numpy().dtype
        )
        prd_gt_overlaps = np.zeros(
            (num_pairs, targets_per_image.get_field(
                'prd_gt_overlaps').shape[1]),
            dtype=targets_per_image.get_field(
                'prd_gt_overlaps').cpu().numpy().dtype
        )
        pair_to_gt_ind_map = -np.ones(
            (num_pairs), dtype=targets_per_image.get_field(
                'pair_to_gt_ind_map').cpu().numpy().dtype
        )

        pair_gt_inds = np.arange(targets_per_image.get_field(
            'prd_gt_classes_minus_1').shape[0])
        if len(pair_gt_inds) > 0:
            sbj_gt_boxes = targets_per_image.get_field(
                'sbj_gt_boxes').cpu().numpy()[pair_gt_inds, :]
            sbj_gt_classes_minus_1 = targets_per_image.get_field(
                'sbj_gt_classes_minus_1').cpu().numpy()[pair_gt_inds]
            obj_gt_boxes = targets_per_image.get_field(
                'obj_gt_boxes').cpu().numpy()[pair_gt_inds, :]
            obj_gt_classes_minus_1 = targets_per_image.get_field(
                'obj_gt_classes_minus_1').cpu().numpy()[pair_gt_inds]
            prd_gt_classes_minus_1 = targets_per_image.get_field(
                'prd_gt_classes_minus_1').cpu().numpy()[pair_gt_inds]
            ...
            pair_maxes = pair_to_gt_overlaps.max(
                axis=1)  # Amount of that overlap
            # Those boxes with non-zero overlap with gt boxes, get all items
            pair_I = np.where(pair_maxes >= 0)[0]
            # Record max overlaps with the class of the appropriate gt box
            sbj_gt_overlaps[sbj_I,
                            sbj_gt_classes_minus_1[sbj_argmaxes[sbj_I]]] = sbj_maxes[sbj_I]
            obj_gt_overlaps[obj_I,
                            obj_gt_classes_minus_1[obj_argmaxes[obj_I]]] = obj_maxes[obj_I]
            prd_gt_overlaps[pair_I,
                            prd_gt_classes_minus_1[pair_argmaxes[pair_I]]] = pair_maxes[pair_I]
            pair_to_gt_ind_map[pair_I] = pair_gt_inds[pair_argmaxes[pair_I]]

        sbj_boxes = sbj_boxes.astype(targets_per_image.get_field(
            'sbj_gt_boxes').cpu().numpy().dtype, copy=False)
        targets_per_image.add_field('sbj_boxes', sbj_boxes)
        targets_per_image.add_field(
            'sbj_gt_overlaps', scipy.sparse.csr_matrix(sbj_gt_overlaps))

        obj_boxes = obj_boxes.astype(targets_per_image.get_field(
            'obj_gt_boxes').cpu().numpy().dtype, copy=False)
        targets_per_image.add_field('obj_boxes', obj_boxes)
        targets_per_image.add_field(
            'obj_gt_overlaps', scipy.sparse.csr_matrix(obj_gt_overlaps))

        prd_gt_classes_minus_1 = -np.ones((num_pairs), dtype=targets_per_image.get_field(
            'prd_gt_classes_minus_1').cpu().numpy().dtype)
        targets_per_image.add_field(
            'prd_gt_overlaps', scipy.sparse.csr_matrix(prd_gt_overlaps))
        pair_to_gt_ind_map = pair_to_gt_ind_map.astype(targets_per_image.get_field(
            'pair_to_gt_ind_map').cpu().numpy().dtype, copy=False)
...
def _add_prd_class_assignments(proposals, targets):
    ...
    for (proposals_per_image, targets_per_image) in zip(proposals, targets):
        sbj_gt_overlaps = targets_per_image.get_field(
            'sbj_gt_overlaps').toarray()
        ...
        obj_gt_overlaps = targets_per_image.get_field(
            'obj_gt_overlaps').toarray()
        ...
        prd_gt_overlaps = targets_per_image.get_field(
            'prd_gt_overlaps').toarray()
